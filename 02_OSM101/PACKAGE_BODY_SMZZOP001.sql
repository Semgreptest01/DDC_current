CREATE OR REPLACE PACKAGE BODY OSM101.smzzop001	as
/************************************************************************/
/*	ＰＧＭ名	：  サービス時間判定									*/
/*	ＰＧＭ ｉｄ	：	smzzop001											*/
...
begin
	...
	--  セキュリティシステムの照会可否チェック（SMM_メニュー単位）
	P_判定結果	:=	smzzop901.main('SMM_メニュー単位');
	if	P_判定結果	!=	smzzop900.C_CHK_00	then
		return;
	end if;
	begin
	...
	exception
		...
		when no_data_found then
			P_判定結果	:=	smzzop900.C_CHK_08;		--no_data_found
			return;
	end;
	...
	if		to_char(C_SYSDATE,'hh24mi')	between	W_開始時間	and	W_終了時間
	 or		to_char(to_number(to_char(C_SYSDATE,'hh24mi'))+2400,'FM0000')
		...
		P_判定結果	:=	smzzop900.C_CHK_00;		--サービス中
	else
		P_判定結果	:=	smzzop900.C_CHK_01;		--サービス時間外
	end if;
exception
	when	others	then
	P_判定結果	:=	smzzop900.C_CHK_09;
end;
end	smzzop001;
/
