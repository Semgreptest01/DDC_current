...
/***********************************************************
 *関数名：getDTime()
 *機能　：現在日付時刻を取得（YYYY-MM-DD HH:MI:SS）
 ***********************************************************/
function getDTime() {
	var rslt = null;
	// 現在日付時刻の取得
	myDate = new Date();
	my_yyyy = myDate.getFullYear();
	wk_mm   = myDate.getMonth()+1;
	H = myDate.getHours();M = myDate.getMinutes();S = myDate.getSeconds();
	my_mm = "";my_dd = "";
	if( wk_mm < 10 ){my_mm = "0" + wk_mm;}else{my_mm = wk_mm;}
	wk_dd = myDate.getDate();
	if( wk_dd < 10 ){my_dd = "0" + wk_dd;}else{my_dd =wk_dd;}
	if( S < 10 ){sec = "0" + S;}else{sec = S;}
	if( H < 10 ){hours = "0" + H;}else{hours = H;}
	if( M < 10 ){min = "0" + M;}else{min = M;}
	ymd = (my_yyyy + "-" + my_mm + "-" + my_dd);
