 ...
 *機能　：少数入力はエラーにする
 ***********************************************************/
function shosuChk( obj ) {
	var wk = "";
	// 先頭がピリオドは、エラーにする。
	if( obj.value != "" ){
		wk = obj.value.substr(0,1);
		if( wk == "." ){
			//F0066 小数点以下の入力はできません。
			errSt( obj );
			dspMsg( "F0066" );
			obj.focus();
			return false;
	...
	// 最後の小数点は取り除く
	wk_array  = obj.value.split(".");
	if( wk_array[1] == "" ){
		obj.value = wk_array[0];
	}
}
/***********************************************************
...
function dtChk( str, dtSlshFlg, alrtFlg ) {
	...
	if ( str.match( dtPttrn ) ) {
		...
		if ( alrtFlg ) {
			if ( dtSlshFlg ) {
				dspMsg( "F0001", new Array( "YYYY/MM/DD" ) );
			} else {
				dspMsg( "F0001", new Array( "YYYYMMDD" ) );
			}
		}
		return false;
	}
...
/***********************************************************
 *関数名：helfNumrcChk( str, alrtFlg )
 *機能　：半角数字チェック
 ***********************************************************/
function helfNumrcChk( str, name, alrtFlg ) {
	if ( str.match( /[^0-9]+/ ) ) {
		if ( alrtFlg ) {
			dspMsg( "F0051", name);
		}
